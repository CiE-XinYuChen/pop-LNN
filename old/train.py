...
data = pd.read_csv('standardized_population.csv')
...
optimizer = optim.Adam(model.parameters(), lr=0.0001)  # Learning rate adjusted to 0.001
# scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=5, gamma=0.35)
...
for epoch in range(num_epochs):
    ...
    # Forward pass
    outputs, _ = model(X_train.unsqueeze(1))  # Unsqueeze to match the input dimensions
...
torch.save(model.state_dict(), 'ltc_population_model.pth')

# No predictions made in this part of the program; this code only calculates loss and saves the model.
